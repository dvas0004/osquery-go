...
// Original: https://github.com/osquery/osquery-go/blob/master/plugin/table/table.go
// Reference for errors: https://github.com/osquery/osquery/blob/master/osquery/sql/virtual_table.cpp
...
// Insert returns the rows generated by the table. The ctx argument
// should be checked for cancellation if the generation performs a
// substantial amount of work. The queryContext argument provides the
// deserialized JSON query context from osquery.
type InsertFunc func(ctx context.Context, valuesArray []interface{}) ([]map[string]string, error)
...
func NewWriteablePlugin(name string, columns []ColumnDefinition, gen GenerateFunc, ins InsertFunc) *Plugin {
	return &Plugin{
		name:     name,
		columns:  columns,
		generate: gen,
...
func (t *Plugin) Call(ctx context.Context, request osquery.ExtensionPluginRequest) osquery.ExtensionResponse {
	...
	case "insert":
		_, err := parseQueryContext(request["context"])
		if err != nil {
			fmt.Println(err)
		...
		err = json.Unmarshal([]byte(request["json_value_array"]), &valuesArray)
		if err != nil {
			fmt.Println(err)
			return osquery.ExtensionResponse{
				Status: &osquery.ExtensionStatus{
		...
		rows, err := t.insert(ctx, valuesArray)
		if err != nil {
			fmt.Println(err)
			return osquery.ExtensionResponse{
				Status: &osquery.ExtensionStatus{
